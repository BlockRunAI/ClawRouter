...
async function main(): Promise<void> {
  ...
  if (args.onramp) {
    const url = await createCoinbaseOnrampUrl(args.onrampAmount);
    if (args.onrampJson) {
  ...
  if (args.chain) {
    ...
    if (targetChain === "solana") {
      const { setupSolana } = await import("./auth.js");
      const { getSolanaAddress } = await import("./wallet.js");
      const resolved = await resolveOrGenerateWalletKey();
      const solanaPrivateKeyBytes =
        resolved.solanaPrivateKeyBytes ?? (await setupSolana()).solanaPrivateKeyBytes;
      const solanaAddr = await getSolanaAddress(solanaPrivateKeyBytes);
      console.log(
