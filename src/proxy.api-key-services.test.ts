...
describe("account API service parity through the proxy", () => {
  ...
  beforeAll(async () => {
    upstream = createServer((req, res) => {
      ...
      res.setHeader("content-type", "application/json");
      res.setHeader("x-blockrun-cost-usd", "0.0075");
  ...
  it("records the new gateway settled-cost header for account passthrough", async () => {
    vi.mocked(logUsage).mockClear();
    const response = await fetch(proxy.baseUrl + "/v1/surf/market/ranking");
    await response.json();
    await vi.waitFor(() =>
      expect(logUsage).toHaveBeenCalledWith(
        expect.objectContaining({ cost: 0.0075, tier: "SURF" }),
      ),
    );
  });
  it.each([
