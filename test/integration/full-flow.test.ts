...
describe.skipIf(!process.env.BLOCKRUN_WALLET_KEY)(
  "ClawRouter full-flow (funded wallet)",
  () => {
    beforeAll(async () => {
    ...
    it(
      "chat completion with blockrun/free returns valid response",
      async () => {
    ...
    it(
      "chat completion with blockrun/auto resolves to a model",
      async () => {
        const res = await fetch(`${getTestProxyUrl()}/v1/chat/completions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
    ...
    it(
      "streaming chat completion returns SSE with data and [DONE]",
      async () => {
        const res = await fetch(`${getTestProxyUrl()}/v1/chat/completions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
