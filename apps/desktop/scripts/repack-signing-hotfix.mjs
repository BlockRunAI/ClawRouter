...
// --window-drag produces 0.1.2 from the checksum-pinned public 0.1.1 DMG.
...
const [image, mountedApp, dependencyPackage, variant] = process.argv.slice(2);
if (variant && variant !== '--window-drag') throw new Error('Unknown repack variant');
const dragFix = variant === '--window-drag';
const version = dragFix ? '0.1.2' : '0.1.1';
if (!image || !mountedApp || !dependencyPackage) throw new Error('Usage: node repack-signing-hotfix.mjs <original.dmg> <mounted/ClawRouter.app> <desktop/package.json with installed build dependencies> [--window-drag]');
...
const expectedHash = dragFix ? '65f71ab76fa100c3bf64eea57d883b222f8eadc0ac3f3f72b9f64bcf02b96e29' : '0b687450d3d621fad0da0cb292dfe528aa963199cfeb9842a1a0f97beed9c954';
if (inputHash !== expectedHash) throw new Error('Unrecognized original DMG');
...
if (pkg.version !== (dragFix ? '0.1.1' : '0.1.0')) throw new Error('Unexpected input version');
pkg.version = version;
await writeFile(pkgPath, JSON.stringify(pkg, null, 2)+'\n');
let dragCssSha256;
if (dragFix) {
  const css = await readFile(new URL('../src/window-drag.css', import.meta.url));
  dragCssSha256 = sha(css);
  await writeFile(join(unpacked, 'dist/window-drag.css'), css);
  const htmlPath = join(unpacked, 'dist/index.html');
  const html = await readFile(htmlPath, 'utf8');
  if (html.split('</head>').length !== 2 || html.includes('window-drag.css')) throw new Error('Unexpected renderer HTML');
  await writeFile(htmlPath, html.replace('</head>', '<link rel="stylesheet" href="./window-drag.css"></head>'));
}
...
for (const key of ['CFBundleShortVersionString', 'CFBundleVersion']) execFileSync('/usr/libexec/PlistBuddy', ['-c', `Set :${key} ${version}`, plist]);
...
execFileSync('/usr/bin/hdiutil',['create','-volname',`ClawRouter ${version}-arm64`,'-srcfolder',stage,'-format','UDZO',...(dragFix ? ['-fs','HFS+','-imagekey','zlib-level=9'] : []),'-ov',dmg],{stdio:'inherit'});
...
await writeFile(join(work,'BUILD-PROVENANCE.json'),JSON.stringify({
  type:dragFix ? 'window-drag-hotfix-repack' : 'signing-only-repack',version,inputHash,originalRelease:dragFix ? 'desktop-v0.1.1-preview.1' : 'desktop-v0.1.0-preview.1',dragCssSha256,
