...
describe("ClawRouterManager adapter flow", () => {
      ...
      const manager = fixtureManager(
        ...
        async (command, args) => {
          if (command === "npx" && args.includes("setup")) {
            ...
            await writeFile(
              path,
              JSON.stringify({ plugins: { blockrun: { baseUrl: "http://127.0.0.1:8402/v1" } } }),
            );
          }
  ...
  it("shows the preferred shared Core wallet and marks a stale proxy wallet for restart", async () => {
    ...
    const dashboard = await manager.dashboard();
    expect(dashboard.proxy.wallet).not.toBe(staleWallet);
  ...
  it("disconnects OpenClaw and Hermes safely when their pre-Desktop configs have no backup", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        plugins: { entries: { clawrouter: { enabled: true }, keep: { enabled: true } } },
      }),
    );
    await writeFile(
...
function fixtureManager(
  ...
  const fetcher = (async (url: string | URL | Request) => {
    ...
    return response({ status: "ok", wallet: "0xabc", paymentChain: "base", ...health });
  }) as typeof fetch;
  return new ClawRouterManager({
    homeDir,
