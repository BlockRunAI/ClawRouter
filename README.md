...
Equivalent flag: `TWZRD_GATE_ENABLED=true`. Requires optional dependency `twzrd-x402-gate@0.9.4`, pinned exactly — it is npm `latest`; `0.10.0` / `0.10.1` are deprecated as unreproducible, do not use them. Forks that omit it still install. Every lookup is stamped `X-Twzrd-Caller: clawrouter/<version>@0.9.4` and `X-TWZRD-Integration: clawrouter/<version>` so a refuse is attributable. Unset the flag to go back to SpendControl only.

**Wash-only, on Solana and Base alike.** `createTwzrdBeforePaymentHook` defaults to the package's wash engine: one `GET merchant_card/{payTo}` to `intel.twzrd.xyz` before each signature, whatever the network. ClawRouter does not select the full preflight engine — no `POST /v1/intel/preflight`, no reputation score — so only the `payTo` leaves your machine, not the resource URL, price or chain. A `wash_flagged` `payTo` aborts before sign (`twzrd_wash_flagged`); a clean `payTo` with full coverage is allowed; a clean-looking card with missing, partial or stale coverage also aborts (`twzrd_wash_unknown`). We bound the call at `TWZRD_GATE_TIMEOUT_MS` (default 2000) and proceed if it does not answer, because SpendControl is the guarantee and a third party must not be able to stop your payments. `TWZRD_FAIL_OPEN=false` makes that timeout (or a thrown gate) refuse instead — but it cannot make the package refuse a fast lookup failure: `0.9.4` turns a non-2xx card, a network error or its own 3 s timeout into allow. Refuse-on-outage is therefore only partly enforced until the package changes.
